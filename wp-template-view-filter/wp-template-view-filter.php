<?php
/**
* Plugin Name: WP Template View Filter
* Plugin URI: https://github.com/chriskleckner
* Description: Adds dropdown to filter by template on Pages admin. Adds Pages capabilities to Author Role. Displays template info in the admin bar for logged in Admins and Authors when viweing pages.
* Version: 1.0
...
function wp_template_view_filter_required_role_caps() {
...
add_action( 'init', 'wp_template_view_filter_required_role_caps', 11 );
...
add_action( 'restrict_manage_posts', 'wp_template_view_filter_dropdown' );
function wp_template_view_filter_dropdown() {
...
add_filter( 'parse_query', 'wp_template_filter_by_selected_template' );
function wp_template_filter_by_selected_template( $query ) {
